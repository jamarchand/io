...
<?php
    ...
    function match_prot($fname, $dl){
        ...
        if ($handle) {
            while (($line = fgets($handle)) !== false) {
                ...
                if ($ans ==1){
                    trim_cluster('UNTRIMMED-CLUSTER.csv',$protA);
                }
    ...
    shell_exec("makeblastdb -in OUTGROUP.fasta -out databases/databaseOUT -dbtype prot -parse_seqids");
    ...
    shell_exec("blastp -query QUERY.fasta -db databases/databaseOUT -outfmt 6 -num_alignments 1 -out outgroup.txt");
